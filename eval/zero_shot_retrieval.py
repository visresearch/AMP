...
def gather_index_tensors(tensor):
    rank = torch.distributed.get_rank()
    world_size = torch.distributed.get_world_size()
    ...
    for i in range(world_size):
        all_tensors[i] = all_tensors[i] + offset
        offset += (all_tensors[i][-1] + 1)
...
def evaluate(model, dataloader, tokenizer, device, amp=True, recall_k_list=[5]):
    ...
    texts_image_index = torch.tensor(texts_image_index).to(images_emb.device)

    if world_size > 1:
        images_emb = gather_tensors(images_emb).cpu()
        texts_emb = gather_tensors(texts_emb).cpu()
        texts_image_index = gather_index_tensors(texts_image_index).cpu()
